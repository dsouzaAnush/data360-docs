...
```json segment_definition.json highlight={3-4}
{
...
<RequestExample>
```sql valid_primary_key.sql icon=check highlight={1}
SELECT Id__c
FROM UnifiedIndividual__dlm
WHERE LifetimeValue__c > 1000
```

```sql invalid_multiple_columns.sql icon=xmark highlight={1}
...
```sql invalid_aggregations.sql icon=xmark highlight={2-4}
...
```sql invalid_wildcard.sql icon=xmark highlight={2}
...
```sql invalid_case.sql icon=xmark highlight={2-3}
...
```sql invalid_alias.sql icon=xmark highlight={2-3}
...
```sql key_qualifiers.sql icon=check highlight={1-2,4}
...
<RequestExample>
```sql valid_qualified.sql icon=check highlight={1,4}
SELECT ui.Id__c
...
```sql invalid_unqualified.sql icon=xmark highlight={2,4}
...
```sql valid_relationship.sql icon=check highlight={3-4}
SELECT ui.Id__c
FROM UnifiedIndividual__dlm ui
...
<RequestExample>
```sql valid_profile_first.sql icon=check highlight={2}
SELECT ui.Id__c
FROM UnifiedIndividual__dlm ui  -- Profile DMO first
JOIN SalesData__dlm sd ON ui.Id__c = sd.CustomerId__c
```

```sql invalid_non_profile_first.sql icon=xmark highlight={2}
...
<RequestExample>
```sql valid_subquery_where.sql icon=check highlight={3-7}
SELECT Id__c
FROM UnifiedIndividual__dlm
WHERE Id__c IN (
  SELECT PartyId__c
...
```sql invalid_subquery_select.sql icon=xmark highlight={2}
...
<RequestExample>
```sql valid_single_column.sql icon=check highlight={2}
-- Valid: Single column in subquery
WHERE Id__c IN (SELECT PartyId__c FROM UnifiedContactPointEmail__dlm)
```

```sql invalid_multiple_columns_sub.sql icon=xmark highlight={2}
...
<RequestExample>
```sql valid_type_matching.sql icon=check highlight={2-4}
...
```sql invalid_type_mismatch.sql icon=xmark highlight={2}
...
```sql pagination.sql icon=check highlight={4-5}
SELECT Id__c
FROM UnifiedIndividual__dlm
...
<RequestExample>
```sql valid_table_alias.sql icon=check highlight={2-3}
SELECT ui.Id__c
FROM UnifiedIndividual__dlm ui
WHERE ui.IsActive__c = true
```

```sql invalid_column_alias.sql icon=xmark highlight={2-3}
...
```sql simple_segment.sql icon=database highlight={1-3}
SELECT Id__c
FROM UnifiedIndividual__dlm
...
```sql segment_with_join.sql icon=database highlight={1-5}
SELECT ui.Id__c
FROM UnifiedIndividual__dlm ui
JOIN UnifiedContactPointEmail__dlm cpe
...
```sql segment_with_subquery.sql icon=database highlight={3-7}
SELECT Id__c
FROM UnifiedIndividual__dlm
WHERE Id__c IN (
  SELECT PartyId__c
...
```sql segment_with_qualifiers.sql icon=database highlight={1-2,4}
SELECT Id__c, DataSource__c
FROM UnifiedIndividual__dlm
