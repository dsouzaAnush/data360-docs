...
```python create_cursor.py icon=python lines highlight={4-9,12}
...
```python basic_query.py icon=python lines highlight={2-6,9-11}
...
```python filtered_query.py icon=python lines highlight={4-5}
cursor.execute("""
...
```python aggregation_query.py icon=python lines highlight={4-7,10}
cursor.execute("""
    SELECT
        Region__c,
...
```python join_query.py icon=python lines highlight={8-10}
cursor.execute("""
    SELECT
        ui.Id__c,
        ui.FirstName__c,
...
```python icon=python lines highlight={4-5}
...
```python icon=python lines highlight={4-7}
...
```python result_metadata.py icon=python lines highlight={4,8}
...
```python safe_query.py icon=python lines highlight={3-5,7-13}
...
```python pagination.py icon=python lines expandable highlight={7-14,16-18,20-22}
...
```python query_error_handling.py icon=python lines highlight={4-8}
...
<AccordionGroup>
  ...
  <Accordion title="Select Only Needed Columns">
    ...
    ```python icon=python lines highlight={3}
...
```python close_cursor.py icon=python lines highlight={3-7,11-13}
# Manual close
cursor = connection.cursor()
try:
