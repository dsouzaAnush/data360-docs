...
The DevOps Data Kit helps you collect, sequence, and deploy Data 360 configuration metadata from sandboxes to production or between environments.

### Why Use It

Data 360 metadata has complex dependencies (e.g., segments depend on DMOs, which depend on data streams). The DevOps Data Kit handles dependency ordering automatically.

### What's Deployable

| Component | Deployable | Notes |
|-----------|-----------|-------|
| **Data Streams** | Yes | Connector configs and mappings |
| **DMO Mappings** | Yes | Field mappings to standard model |
| **Identity Resolution Rulesets** | Yes | Match and reconciliation rules |
| **Calculated Insights** | Yes | SQL-based insight definitions |
| **Segments** | Yes | Segment criteria and SQL rules |
| **Data Transforms** | Yes | Batch and streaming definitions |
| **Data Actions** | Yes | Rules and target configurations |
| **Activation Targets** | Partial | Target configs (credentials need reconfiguration) |

### Deployment Process

<Steps>
  <Step title="Prepare the Data Kit">
    In your sandbox, navigate to **Setup > DevOps Data Kit** and create a new kit. Select the components to include.
  </Step>
  <Step title="Review Dependencies">
    The data kit automatically identifies and includes dependent components. Review the component list.
  </Step>
  <Step title="Export">
    Export the data kit as a deployable package.
  </Step>
  <Step title="Deploy to Target">
    Use one of the deployment methods to apply the kit to the target org.
  </Step>
  <Step title="Reconfigure Credentials">
    Re-authenticate external connectors and update credentials in the target org.
  </Step>
</Steps>
