...
<Steps>
  <Step title="Define the Data Stream">
    ...
    ```bash icon=terminal lines highlight={1-3}
  ...
  <Step title="Configure Field Mappings">
    ...
    ```json field_mapping.json highlight={3-8}
    {
  ...
  <Step title="Ingest Data">
    ...
    ```python ingest_subscribers.py icon=python lines highlight={3,5-12,14-15}
    import requests
  ...
  <Step title="Verify Mapping">
    ...
    ```sql verify_mapping.sql highlight={1-3}
...
<Steps>
  <Step title="Submit Async Query">
    ...
    ```python submit_query.py icon=python lines highlight={5-6,8-17,19-22}
    import requests
    import time
  ...
  <Step title="Poll for Completion">
    ...
    ```python poll_status.py icon=python lines expandable highlight={1-4,8-12,14-15,20-21}
  ...
  <Step title="Retrieve Results with Pagination">
    ...
    ```python fetch_results.py icon=python lines expandable highlight={1-5,9-11,13-14,16-17,23-24}
  ...
  <Step title="Process Results">
    ...
    ```python process_results.py icon=python lines highlight={3-5,7-11,15}
    import pandas as pd
...
<CodeGroup>
```python realtime_lookup.py icon=python lines highlight={3-4,6-19,21-30,33-36}
import requests
...
```java CustomerLookup.java icon=java lines highlight={5-17,19-26}
import java.sql.*;
...
```python monitor_activations.py icon=python lines expandable highlight={5-10,12-14,17-24,26-32,35-40}
import requests
